...
// Fix #1: Sort directory entries to ensure deterministic ingestion order
// across OS/filesystems that do not guarantee readdir ordering.
...
async function parseToText(filePath: string): Promise<string> {
  ...
  if (ext === ".pdf") {
    ...
    // Fix #3: ensure parser.destroy() is called even if parsing fails,
    // using a finally block so errors from destroy do not mask parse failures.
    let parser: import("pdf-parse").PDFDocumentProxy | undefined;
    try {
      ...
      if (parser !== undefined) {
        try {
          // @ts-expect-error destroy may not be typed on all versions
...
export async function ingestKnowledgeBase(config: IngestionConfig): Promise<IngestionSummary> {
  ...
  for (const filePath of files) {
    ...
    // Fix #2: normalize to POSIX-style separators so chunk IDs are OS-independent.
    const relPosix = rel.split(path.sep).join("/");
    chunks.forEach((chunk, idx) => {
      const chunkId = `${relPosix}:${idx}`;
